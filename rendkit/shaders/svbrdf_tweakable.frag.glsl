...
void main() {
    ...
    vec3 alb_d = lab2rgb(texture2D(diff_map, uv).xyz * source_std + source_mean);
    vec3 alb_s = texture2D(spec_map, uv).rgb * spec_scale;
    vec3 specv = texture2D(spec_shape_map, uv).rgb * spec_shape_scale;

    vec3 E = normalize(u_cam_pos - v_position);

    mat3 TBN = mat3(v_tangent, v_bitangent, v_normal);
    vec3 N = normalize(TBN * texture2D(normal_map, uv).rgb);
    mat3 R = mat3(0, 0, N.x,
                  0, 0, N.y,
                  -N.x, -N.y, 0);
	...
	for (int i = 0; i < NUM_LIGHTS; i++) {
		vec3 L = u_light_position[i] - v_position;
		float D2 = dot(L, L);
		L = L / sqrt(D2);
		...
		vec3 radiance = (vec3(spec) * alb_s + alb_d)
				* vec3(ndotl) / D2 * u_light_intensity[i] * u_light_color[i];
		total_radiance += radiance;
	}
	total_radiance += alb_d * vec3(0.1, 0.1, 0.1);
//	total_radiance = total_radiance / (total_radiance + 1.0);
